 ...
 * ============================================
 * Security Fixes:
 * - CSRF protection on all AJAX actions
 * - Authorization checks
 * - Input validation
 * - Rate limiting
 * - Proper sanitization layers
 */
...
// --- Main Page Load ---
// SECURITY FIX #3: Validate profile user ID as integer
...
try {
    ...
    // SECURITY FIX #4: Validate pagination inputs
...
function handleAjaxRequest($action, $pdo) {
    ...
    // SECURITY FIX #5: CSRF validation on ALL actions
    ...
    switch ($action) {
        case 'update_bio':
            ...
            // SECURITY FIX #8: Don't use htmlspecialchars in DB layer
        ...
        case 'follow':
            // SECURITY FIX #9: Rate limiting on follows
            ...
            // SECURITY FIX #10: Validate target_id
            ...
            // SECURITY FIX #11: Check if target user exists
            ...
            // SECURITY FIX #12: Check if already following (explicit check)
            $stmt = $pdo->prepare('
            ...
            if ($result) {
                ...
                $stmt = $pdo->prepare('
                    ...
                    VALUES (?, "follow", ?, "You now share light with this soul.", NOW())
                ');
        ...
        case 'unfollow':
            // SECURITY FIX #13: Validate target_id
            $target_id = filter_var($_POST['target_id'] ?? null, FILTER_VALIDATE_INT);
            if (!$target_id) {
                echo json_encode(['success' => false, 'message' => 'Invalid target']);
                return;
        ...
        case 'like_post':
            // SECURITY FIX #14: Rate limiting on likes
            ...
            // SECURITY FIX #15: Validate post_id
            ...
            // SECURITY FIX #16: Check if post exists and is published
            ...
            if ($already_liked) {
                ...
                if ($post_owner !== $user_id) {
                    $stmt = $pdo->prepare('
                        ...
                        VALUES (?, "like", ?, ?, "Someone loves your story.", NOW())
                    ');
...
<head>
    ...
    <!-- SECURITY FIX #17: Add CSRF token meta tag -->
...
<body data-theme="<?php echo htmlspecialchars($preferences['energy_mode']); ?>" data-user-id="<?php echo $current_user_id ?? ''; ?>">
    ...
    <div class="profile-container">
        ...
        <!-- Cover Image -->
        <!-- SECURITY FIX #18: Sanitize theme_color output -->
            ...
            <div class="header-content">
                ...
                <div class="avatar-section">
                    <div class="avatar-aura" style="border-color: <?php echo htmlspecialchars($profile_user['theme_color'] ?? '#667EEA'); ?>;">
                        <img src="<?php echo htmlspecialchars($profile_user['avatar'] ?? '/assets/images/default-avatar.png'); ?>" 
