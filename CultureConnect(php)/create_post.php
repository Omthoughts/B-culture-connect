...
// FIX: Ensure $pdo is available (in case database.php doesn't expose it globally)
if (!isset($pdo)) {
    die('Database connection not available');
}

// FIX: Define helper functions if they don't exist
...
if (!function_exists('security') || !security()->requireAuth()) {
    // Fallback if security() function doesn't exist
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_post'])) {
    ...
    if (function_exists('security')) {
        $csrf_valid = security()->validateCSRFToken($_POST['csrf_token'] ?? '');
    } else {
    ...
    // Rate limiting: 5 posts per hour
        ...
        $rate_limit_ok = true;
        if (function_exists('security')) {
            $rate_limit_ok = security()->checkRateLimit('create_post:' . $user_id, 5, 3600);
        }
                ...
                if (isset($_FILES['media']) && $_FILES['media']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (!in_array($actualMimeType, $allowedTypes, true)) {
                        ...
                        if ($imageInfo === false) {
                            ...
                            // FIX: Replace match() with switch for PHP 7.x compatibility
                            ...
                            if (class_exists('ImageHelper') && ImageHelper::optimizeAndSave($_FILES['media']['tmp_name'], $filepath)) {
                                $media_url = '/uploads/posts/' . date('Y/m/') . $filename;
                                $media_type = 'image';
                            } elseif (move_uploaded_file($_FILES['media']['tmp_name'], $filepath)) {
                                // Fallback if ImageHelper doesn't exist
                                $media_url = '/uploads/posts/' . date('Y/m/') . $filename;
                                $media_type = 'image';
                            } else {
...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM posts WHERE user_id = ?');
$stmt->execute([$user_id]);
$user_post_count = (int)$stmt->fetchColumn();
    ...
    <div class="create-container">
        ...
        <header class="create-header">
            ...
            <p class="create-subtitle">
                ...
                <?php else: ?>
                    <?= $user_post_count ?> stories shared. The world is listening. 🌏
                <?php endif; ?>
            </p>
        </header>
